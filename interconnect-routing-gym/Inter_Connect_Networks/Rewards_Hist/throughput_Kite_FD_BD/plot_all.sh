...
for line in `cat  Q_c.txt`
do
	reward=$(echo "scale=4; $line + 20" | bc)
	echo $reward >> Q_minus.txt
done

for line in `cat  sarsa_c.txt`
do
	reward=$(echo "scale=4; $line + 15" | bc)
	echo $reward >> sarsa_minus.txt
done

for line in `cat  esarsa_c.txt`
do
	reward=$(echo "scale=4; $line + 30" | bc)
