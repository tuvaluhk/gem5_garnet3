...
path = '/home/tuvalu/Documents/HeteroGarnet-early/'
extnet = Extract_Network(path) # TL added 072520

# topologies = ["Mesh_XY", "Mesh_XY", "Mesh_westfirst"]
# routing_algorithm = ["0", "0", "0"]
#actions = ["NoI_Mesh", "NoI_CMesh", "NoI_ButterDonut_X" "NoI_KiteSmall" "NoI_DoubleButterfly_X" "NoI_FoldedTorus_X"]
actions = ["NoI_ButterDonut_X", "NoI_KiteSmall", "NoI_FoldedTorus_X"]
...
def throughput_f(d): ## TL added 090620
	throughput = float(d["flits_received"][-1]) * 16.0 * 40 / float(d["sim_seconds"][-1]) / 1e+9
	return throughput
...
def icn_routing_gym(ith_eps, injrate, action, topology):     
	...
	os_command = "/home/tuvalu/Documents/HeteroGarnet-early/build/NULL/gem5.debug -d /home/tuvalu/Documents/HeteroGarnet-early/m5out/ \
	                               /home/tuvalu/Documents/HeteroGarnet-early/configs/my/garnet_synth_traffic1.py \
					--network=garnet2.0 --num-cpus=64 --num-dirs=16 --topology={} --mesh-rows=8 --num-chiplets=4 \
					--sim-cycles=20000 --vcs-per-vnet=4 --inj-vnet=0 --injectionrate={:f} \
					--synthetic=uniform_random --routing-algorithm=0".format(action, injrate) 
